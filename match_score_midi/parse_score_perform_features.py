...
sys.path.append('/workspace/Piano/gen_task/')
sys.path.append('/workspace/Piano/gen_task/match_score_midi')
import numpy as np
...
from match_score_midi.main import XML_SCORE_PERFORM_MATCH as MATCH
from match_score_midi.parse_utils import *
import piano_cvae_data2 as pdata
...
def make_dir_from_crawled(categ):
    data_path = '/workspace/Piano/gen_task/parse_features/crawled_data/chopin_cleaned/{}'.format(categ)
    parent_path = '/data/chopin_cleaned/original/{}'.format(categ)
    ...
    for piece in pieces:
        ...
        #    continue
        players = sorted(glob(os.path.join(data_path, p_name, '*')))
        for n, player in enumerate(players):
            pl_name = os.path.basename(player)
            new_dir = os.path.join(piece, ind2str(n+1, 2))
...
def make_dir_from_pro_midi():
    data_path = '/workspace/Piano/gen_task/chopin_pro_midi'
    parent_path = '/data/chopin_cleaned/original/Chopin_Etude'
    pieces = sorted(glob(os.path.join(parent_path, "*/")))

    for piece in pieces:
        p_name = piece.split("/")[-2]
        players = sorted(glob(os.path.join(data_path, '*_{}.mid'.format(p_name))))
        for n, player in enumerate(players):
            pl_name = "{}.pro.mid".format(p_name)
            new_dir = os.path.join(piece, 'pro')
            if not os.path.exists(new_dir):
                os.makedirs(new_dir)
            new_path = os.path.join(new_dir, pl_name)
            shutil.copy(player, new_path)
    ...
    ################ ERASE COPIED DATA ##################
    # for piece in pieces:    
    #     pro_ = sorted(glob(os.path.join(piece, '*.pro.mid')))
    #     # print(pro_)
    #     if len(pro_) == 1:
    #         os.remove(pro_[0])
    #####################################################
    ...
    * Function to search 4 kinds of files in 'dirname'
    ...
    for c in categs:
        ...
        for p in pieces: 
            ...
            xml_path = os.path.join(p, "musicxml_cleaned_plain.musicxml")
            score_path = os.path.join(p, "score_plain.mid")
            perform_path = os.path.join(p, "pro/{}.pro.mid".format(p_name))
            # assign paths to corresponding piece category
            xml_list[c_name][p_name] = xml_path
            score_midi_list[c_name][p_name] = score_path
            perform_midi_list[c_name][p_name] = perform_path

    return xml_list, score_midi_list, perform_midi_list

def get_performance_num():
    dirname = '/data/chopin_cleaned/original'
    xml_list, score_midi_list, perform_midi_list = search(dirname)
    pl_num = 0
    tempo_dict = dict()
    for c in perform_midi_list:
        for p in perform_midi_list[c]:
            players = perform_midi_list[c][p]
            pl_num += len(players)
            cond = np.load(os.path.join(os.path.dirname(
                os.path.dirname(players[0])), 'cond.npy'))
            tempo = cond[0][1][0]
            if len(players) < 5:
                print(p, tempo, len(players))
            try:
                tempo_dict[tempo] += 1
            except KeyError:
                tempo_dict[tempo] = 1
...
def save_matched_files():
    ...
    dirname = '/data/asap_dataset/original'
    ...
    for categ in sorted(perform_midi_list): 
        for piece in sorted(perform_midi_list[categ]):
            ...
            if os.path.exists(pair_path) is False:
                try:
                    _, _ = match(xml, score, performs, save_pairs=True)	
                except:
...
def save_features_xml():
    ...
    for categ in categs:
        ...
        for piece in pieces:
            ...
            if os.path.exists(os.path.join(piece, 'cond.npy')) is False:
                # PARSE FEATURES
                cond_list = list()
                note_list = list()
                csv_list = list()
                ...
                for i in range(len(pairs_xml)):
                    ...
                    parsed_note = XMLFeatures(note=xml_note,
                                            ...
                                            prev_wedge_in_middle=prev_wedge_in_middle,
                                            cresc_in_word=cresc_in_word,
                                            dim_in_word=dim_in_word,
                                            cresc_in_wedge=cresc_in_wedge,
                                            dim_in_wedge=dim_in_wedge,
                                            note_ind=i)
                    ...
                    csv_list.append(["--> downbeat input: {}".format(_input[3:5])])
                    csv_list.append([])
                    csv_list.append([])
                    ...
                    prev_wedge_in_middle, \
                    cresc_in_word, dim_in_word, cresc_in_wedge, dim_in_wedge = \
                        parsed_note.wedge_in_middle, \
                        parsed_note.cresc_in_word, parsed_note.dim_in_word, parsed_note.cresc_in_wedge, parsed_note.dim_in_wedge
...
def save_features_midi():
    '''
    <Chopin cleaned> (updated 210422)
    - matched note number: 808359 notes (dataset)
    ...
    * only Etude: 
    ...
    for categ in categs:
        ...
        for piece in pieces:
            p_name = piece.split('/')[-2]
            players = sorted(glob(os.path.join(piece, '*/')))
            if os.path.exists(os.path.join(players[0], 'inp_.npy')) is False:
                ...
                for player in players:
                    ...
                    input_list, output_list = parse_midi_features(
                        pair_path=pair_path, midi_path=midi_path, 
                    ...
                    # np.save(os.path.join(player, 'oup2.npy'), output_list) # pooled ioi
...
def parse_test_cond(pair=None, pair_path=None):
    ...
    for i in range(len(pairs_xml)):
        ...
        # parse features for each note
        parsed_note = XMLFeatures_test(note=xml_note,
                                         measure=xml_measure,
                                         prev_note=prev_xml_note,
                                         prev_measure=prev_xml_measure,
                                         note_ind=i)
...
def parse_midi_events(midi_path=None, midi_notes=None):

    ## FOR DEBUGGING ## 
    # midi_path = '/data/chopin_cleaned/original/Chopin_Etude/10_1/score_plain.mid'
    # midi_path = '/data/chopin_cleaned/original/Chopin_Etude/10_1/01/Avdeeva02.mid'
    # midi_path = '/data/chopin_cleaned/original/Chopin_Etude/25_8/score_plain.mid'
    # midi_path = '/data/chopin_cleaned/original/Chopin_Etude/25_8/01/DANILO04_8.mid'

    # get score midi for check
    if midi_path is not None:
        midi_notes, _ = extract_midi_notes(midi_path, clean=False)
    if midi_notes is not None:
        midi_notes = midi_notes	

    # initiate
    parsed = MIDIEvents(notes=midi_notes)
    # parse index
    idxs = parsed()
    # events = parsed.from_array(idxs)
    # notes = parsed.event2note(events)

    return idxs

def parse_midi_features(
    pair_path=None, pairs_score=None, 
    midi_path=None, midi_notes=None, 
        ...
        if parsed_note.is_same_onset is False: # different note group
            ...
            * next note to current group's last note 
            ...
            if same_onset_last_ind < last_pair_ind:
                next_onset = list()
                n = 1
                while len(next_onset) == 0:
                    for onset in pairs_onset:
                        if same_onset_last_ind+n in [o['score_midi'][0] for o in onset]:
...
def parse_test_x_features(
    ...
    if measures is not None and sec is None:
        ...
        xml_notes = extract_xml_notes(XMLDocument)
...
def parse_test_y_features(
    xml=None, score=None, perform=None, corresp=None, x_=None, 
    pairs=None, measures=None, tempo=None, null_tempo=120, same_onset_ind=None):
    ...
    if pairs is None:
        ...
        match = MATCH(
            ...
            program_dir="/workspace/Piano/gen_task/match_score_midi")
        pairs_, _ = match(
            xml, score, perform, corresps=corresp, save_pairs=False)
        pairs = [p for k, p in pairs_.items()][0]
    elif pairs is not None:
        pairs = np.load(pairs, allow_pickle=True).tolist()

    pairs_score_ = [p for p in pairs if p['score_midi'] is not None]
    pairs_score_ = sorted(pairs_score_, key=lambda x: x['score_midi'][0])
    note_ind = [n['score_midi'][0] for n in pairs_score_]
    ...
    if measures is not None:
        ...
        for note in pairs_score_:
            ...
            if measure_num >= start_measure and measure_num <= end_measure:
                pairs_score.append(note)
        pairs_score = sorted(pairs_score, key=lambda x: x['score_midi'][0])
        x_notes = np.asarray([x_[n['score_midi'][0]] for n in pairs_score])
        note_ind = [n['score_midi'][0] for n in pairs_score]

    elif measures is None:
        pairs_score = pairs_score_
        x_notes = x_
        note_ind = note_ind

    # parse output features
    _, output_list_ = parse_midi_features(
        pairs_score=pairs_score, midi_notes=x_notes, 
    ...
    # loc = np.clip(output_list[:,1], -0.1, 0.1)
    # dur = np.clip(output_list[:,2], 0.125, 8)
    # ioi = np.clip(output_list[:,3], 0.125, 8)
    # output_list[:,0] = vel
    # output_list[:,1] = loc
    # output_list[:,2] = dur
    # output_list[:,3] = ioi
    ...
    out = output_list
    # out2 = pdata.make_onset_based_pick(
    # 	x_notes, output_list[:,3], same_onset_ind=same_onset_ind)
    # out3 = pdata.make_onset_based_pick(
    # 	x_notes, output_list[:,4], same_onset_ind=same_onset_ind)

    # return out, np.asarray(out2), np.asarray(out3), x_notes, pairs_score, note_ind
    return out, x_notes, pairs_score, note_ind
...
class MIDIFeatures(object):
    ...
    def get_output_features(
        self, onset_for_ioi, mean_onset, next_onset_for_ioi): #, pooled_ioi
        '''
        outputs based on performed attributes
        '''
        self.get_velocity() # velocity value
        self.get_ioi_ratio(onset_for_ioi, mean_onset, next_onset_for_ioi) # ioi ratio
    ...
    def output_to_vector(self):
        ...
        # self._output = np.stack(
            # [_velocity, _dur_ratio, _ioi_ratio1, _ioi_ratio2], axis=-1)
    ...
    def get_top_voice(self):
        if self.prev_note is None:
            if self.next_note is not None:
                next_onset = Decimal(str(self.next_note.start))
                if self.score_onset == next_onset: # note presents above
                    self.is_top = False
                elif self.score_onset < next_onset: 
                    self.is_top = True
                elif self.score_onset > next_onset:
                    raise AssertionError("invalid onset! (score_onset > next_onset)")
            elif self.next_note is None:
            ...
            if self.next_note is not None:
                ...
                if self.score_onset == next_onset: # note presents above
                    self.is_top = False
                elif self.score_onset < next_onset: # maybe top?
                    if self.score_onset < self.prev_note.base_offset: # note in the middle
                        if self.pitch < self.prev_note.base_pitch: # top note is long
                            self.is_top = False
                        elif self.pitch >= self.prev_note.base_pitch:
                            self.is_top = True
                    elif self.score_onset >= self.prev_note.base_offset: 
                        self.is_top = True
                elif self.score_onset > next_onset:
                    raise AssertionError("invalid onset! (score_onset > next_onset)")
    ...
    def get_ioi_class(self):
        ...
        # self.ioi_units = round(self.score_ioi1, 3) / self.dur_16th
    ...
    # 	# self.dur_units = round(self.score_dur, 3) / self.dur_16th
    ...
    def get_ioi_ratio_new(self, onset_for_ioi, next_onset_for_ioi):
        '''
        ratio of perform midi ioi / score midi(norm) ioi 
        '''
        # previous onset for computing ioi
        ...
        if self.ioi_ratio1 <= 0 or self.ioi_ratio2 <= 0.:
            print("** perform ioi ratio <= 0 !!")
            print(self.note_ind, self.ioi_ratio1, self.base_onset_perform, self.perform_onset)
            print(self.note_ind, self.ioi_ratio2, self.perform_onset, self.next_onset_perform)
...
class XMLFeatures(object):
    ...
    def get_features(self):
        ...
        self.in_measure_pos += self._type["duration"]
    ...
    def features_to_onehot(self):
        ...
        self._input = np.concatenate([_tempo, _staff, _downbeat], axis=-1)
    ...
    def get_beat(self):
        ...
        if self.prev_note is None:
            ...
            beat_unit = self.measure_duration // 4 # 4/4 
            if self.prev_time_position != self.time_position:
                # if first beat
                if self.in_measure_pos == 0:
                    self.beat = 0 # downbeat(beat1) 
                elif self.in_measure_pos < beat_unit: 
                    self.beat = 1 # beat1 ~ beat2 
                elif self.in_measure_pos == beat_unit:
                    self.beat = 2 # beat2  
                elif self.in_measure_pos >= beat_unit and \
                    self.in_measure_pos < beat_unit*2: 
                    self.beat = 3 # beat2 ~ beat3 
                elif self.in_measure_pos == beat_unit*2: 
                    self.beat = 4 # beat3 
                elif self.in_measure_pos >= beat_unit*2 and \
                    self.in_measure_pos < beat_unit*3: 
                    self.beat = 5 # beat3 ~ beat4 
                elif self.in_measure_pos == beat_unit*3: 
                    self.beat = 6 # beat4 					
                elif self.in_measure_pos >= beat_unit*3: 
                    self.beat = 7
...
class XMLFeatures_test(XMLFeatures):
    def __init__(self, 
                 note=None, 
                 measure=None,
                 prev_note=None,
    ...
    def features_to_onehot(self):
        ...
        # concatenate all features into one vector
        self._input = np.concatenate([_staff, _downbeat], axis=-1)
...
DEFAULT_TIME_SHIFT_BINS = np.linspace(0.004, 0.1, num=25)
DEFAULT_VELOCITY_STEPS = 32
DEFAULT_NOTE_LENGTH = BEAT_LENGTH * 2
MIN_NOTE_LENGTH = BEAT_LENGTH / 12 # 48th
...
class MIDIEvents(object):
    ...
    def __init__(self, notes=None):
        # Inputs
        self.notes = notes
        self.time_shift_bins = DEFAULT_TIME_SHIFT_BINS

        self.pitch_range = DEFAULT_PITCH_RANGE
        self.velocity_range = DEFAULT_VELOCITY_RANGE
        self.velocity_steps = DEFAULT_VELOCITY_STEPS
        self.time_shift_bins = DEFAULT_TIME_SHIFT_BINS


    def __call__(self):
        # midi note to index data
        events = self.note2event()
        q_events = self.quantize_time_shifts(events)
        idxs = self.to_array(q_events)
        return idxs

    def get_velocity_bins(self):
        n = self.velocity_range.stop - self.velocity_range.start
        return np.arange(self.velocity_range.start,
                         self.velocity_range.stop,
                         n / (self.velocity_steps - 1)) # 31 bins

    def note2event(self):
        note_events = list()

        if USE_VELOCITY:
            velocity_bins = self.get_velocity_bins()

        for note in self.notes:
            if note.pitch in self.pitch_range:
                if USE_VELOCITY:
                    velocity = note.velocity
                    velocity = max(velocity, self.velocity_range.start)
                    velocity = min(velocity, self.velocity_range.stop - 1)
                    velocity_index = np.searchsorted(velocity_bins, velocity)
                    note_events.append(Event('velocity', note.start, velocity_index))

                pitch_index = note.pitch - self.pitch_range.start
                note_events.append(Event('note_on', note.start, pitch_index))
                note_events.append(Event('note_off', note.end, pitch_index))

        note_events.sort(key=lambda event: event.time) # sort by time
        events = []
        ...
        for i, event in enumerate(note_events):
            ...
            while interval - shift >= self.time_shift_bins[0]:
                index = np.searchsorted(self.time_shift_bins,
                                        interval - shift, side='right') - 1
                '''
                minus 1 b/c np.searchsorted pick index to "insert" target value to the array
                -> if bigger than time_shift_bins[-1], gives index of len(time_shift_bins)
                   which causes IndexError 
                -> if smaller than time_shift_bins[0], while loop stops
                   which does not append "time_shift" event 
                '''
                events.append(Event('time_shift', event.time + shift, index))
                shift += self.time_shift_bins[index]
    ...
    def event2note(self, events):
        ...
        for event in events:
            if event.type == 'note_on':
                pitch = event.value + self.pitch_range.start
                ...
                time += self.time_shift_bins[event.value]
    ...
    def quantize_time_shifts(self, events):
        for event in events:
            assert isinstance(event, Event)

        q_events = copy.deepcopy(events)
    ...
    def feat_dims(self):
        ...
        feat_dims['note_on'] = len(self.pitch_range)
        feat_dims['note_off'] = len(self.pitch_range)
        feat_dims['velocity'] = self.velocity_steps
        feat_dims['time_shift'] = len(self.time_shift_bins)
        return feat_dims

    def feat_ranges(self):
        offset = 0
        feat_ranges = collections.OrderedDict()
        for feat_name, feat_dim in self.feat_dims().items():
            feat_ranges[feat_name] = range(offset, offset + feat_dim)
            offset += feat_dim
        return feat_ranges

    def to_array(self, events):
        feat_idxs = self.feat_ranges()
        idxs = [feat_idxs[event.type][event.value] for event in events]
        dtype = np.uint8 if self.dim() <= 256 else np.uint16
        return np.array(idxs, dtype=dtype)

    def from_array(self, idxs):
        time = 0
        events = []
        for event_index in idxs:
            for event_type, feat_range in self.feat_ranges().items():
                if feat_range.start <= event_index < feat_range.stop:
                    ...
                    if event_type == 'time_shift':
                        time += self.time_shift_bins[event_value]
                    break
        return events

    def dim(self):
        return sum(self.feat_dims().values())
